      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-theme mt-1">
            Comprehensive performance metrics and user insights.
          </p>
        </div>
        <div className="flex gap-3">
          <button className="px-4 py-2 panel-surface-soft hover:bg-[var(--color-hover)] rounded-xl text-xs font-bold transition-all border border-theme">
            Export CSV
          </button>
          <button className="px-4 py-2 bg-neon-pink text-theme rounded-xl text-xs font-bold neon-glow-pink transition-all">
            Live View
          </button>
        </div>
      </div>
      ...
      <div className="glass-card p-8">
        <div className="flex items-center justify-between mb-10">
          <div>
            ...
            <p className="text-muted-theme opacity-40 text-xs mt-1">
              Monthly revenue growth with projection analysis.
            </p>
          </div>
          <div className="flex gap-2">
            <button className="px-3 py-1.5 panel-surface-soft hover:bg-[var(--color-hover)] rounded-lg text-[10px] font-bold transition-colors">
              6 Months
            </button>
            <button className="px-3 py-1.5 bg-neon-purple/20 text-neon-purple border border-neon-purple/30 rounded-lg text-[10px] font-bold transition-colors">
              1 Year
            </button>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <Bar dataKey="newUsers" radius={[4, 4, 0, 0]} barSize={32}>
                    {userAcquisition.map((entry, index) => (
                      <Cell
                        key={`cell-${index}`}
                        fill={
                          index === userAcquisition.length - 1
                            ? "#00F5FF"
                            : "var(--color-muted)"
                        }
        ...
        <div className="glass-card p-6">
          <div className="flex items-center justify-between mb-8">
            ...
            <button className="text-[10px] font-bold text-neon-pink hover:underline uppercase tracking-widest">
              View All
            </button>
          </div>
            ...
            {recentEvents.map((event) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-[11px] font-bold group-hover:text-neon-pink transition-colors">
                      {event.user}
                    </p>
                    <p className="text-[10px] text-muted-theme opacity-40">
                      {event.event}
                    </p>
                  </div>
                </div>
