    ...
    <div className="glass-card p-5">
      ...
      <div className="h-40 w-full relative">
        {mounted ? (
          ...
          <div className="w-full h-full panel-surface-soft animate-pulse rounded-full" />
        )}
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 text-center">
          <div className="w-8 h-8 panel-surface-strong rounded-lg flex items-center justify-center mx-auto mb-1">
    ...
    <div className="glass-card p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2">
          <div className="w-4 h-4 rounded panel-surface-soft flex items-center justify-center text-[8px]">{'<'}</div>
          <div className="w-4 h-4 rounded panel-surface-soft flex items-center justify-center text-[8px]">{'>'}</div>
        </div>
      </div>
      <div className="grid grid-cols-7 gap-1 text-center">
        ...
        {Array.from({length: 31}).map((_, i) => (
          <span key={i} className={cn(
            "text-[9px] py-1 rounded cursor-pointer hover:bg-[var(--color-hover)] transition-colors",
            i + 1 === 24 ? "bg-neon-pink text-theme font-bold neon-glow-pink" : "text-muted-theme"
          )}>
            {i + 1}
          </span>
        ))}
